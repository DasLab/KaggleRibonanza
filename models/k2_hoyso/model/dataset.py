...
class RNADataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        mfe = get_feature('mfe_eternafold', seq)
        loop_type = get_feature('sstype_bprna_eternafold', seq)

        seq_len = len(seq)

        mask = torch.ones(seq_len, dtype=torch.bool)
        
        bpp = torch.tensor(np.half(np.triu(get_feature('bpps_linearfolde', seq))))
        ...
        capr_loop_type = parse_capr(get_feature('sstype_capr', seq))
        ...
        ids = torch.from_numpy(np.arange(id_min,id_max+1))
        ...
        return {'seq':seq, 'mfe':mfe, 'loop_type':loop_type, 'capr_loop_type':capr_loop_type, 'bpp_features':bpp_features, 'bpp':bpp, 'mask':mask}, {'react':react,  'react_err':react_err, 'mask':mask, 'snr':snr, 'ids': ids}
