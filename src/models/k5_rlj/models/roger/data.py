...
class RNA_DS(Dataset):
    ...
    def get_bpp(self, r):
        bpp = np.float32(get_feature('bpps_linearfolde', r.seq))
    ...
    def __getitem__(self, idx: int):
        r = self.df.iloc[idx]
        out = self.get_seq(r) | self.get_bpp(r)
        ...
        ids = np.pad(ids,(1, self.len_max+1-len(r.seq)), constant_values=-1)
        
        return self.pad(out) | self.get_mask(r), {'ids': ids}
