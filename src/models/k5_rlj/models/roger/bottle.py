...
class RNA_Lightning(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        self.update_LR()
        return self.fit_forward(self(batch[0]), batch)

    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        x = self(batch[0])
        if dataloader_idx:
            mask, lmax = batch[0]["mask"].sum(1), batch[0]["seq"].size(1)
    ...
    def predict_step(self, batch, batch_idx, dataloader_idx=0):
        Lmax = batch[0]["mask"].sum(-1).max().long()
        batch[0]["mask"] = batch[0]["mask"][:, :Lmax]
        batch[0]["seq"] = batch[0]["seq"][:, :Lmax]
        batch[0]["bpp"] = batch[0]["bpp"][:, :Lmax, :Lmax]
        batch[1]["ids"] = batch[1]["ids"][:, :Lmax]
        x, mask = self(batch[0]), batch[0]["mask"].to(torch.bool)
        if dataloader_idx:
            mask = mask.flip(1)
