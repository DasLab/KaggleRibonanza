...
class RNA_DS(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        ids = np.arange(r.id_min, r.id_max+1)
        ids = np.pad(ids,(1, self.len_max+1-len(seq)), constant_values=-1)
        ...
        ###################################################################
        if self.infer:
            As_eterna = np.float32(get_feature('bpps_linearfolde', r.sequence))
        ...
        if self.infer:
            return {"seq": self._pad(seq), "As": As, "mask": mask}, {'ids': ids}
        ...
        out = {"seq": self._pad(seq), "As": As, "mask": mask, "react": self._pad(react)}
        ...
        return out | aux, {'ids': ids}

def collate_fn(samples):
    list_keys = list(samples[0][0].keys())
    output = {}
    truncated_output = {}
    for k in list_keys:        
        output[k] = torch.stack([sample[0][k] for sample in samples], 0)
    ...
    list_keys_y = list(samples[0][1].keys())
    output_y = {}
    for k in list_keys_y:
        output_y[k] = torch.stack([torch.tensor(sample[1][k]) for sample in samples], 0)[:,:max_len]
    
    return truncated_output, output_y
