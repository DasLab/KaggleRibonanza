...
class RowwiseDropout(nn.Module):
    def __init__(self, p=0.5):
        super(RowwiseDropout, self).__init__()
        self.p = p

    def forward(self, x):
        if self.training:
            dropout_mask = torch.rand(1, 1, x.size(2), x.size(3), device=x.device) > self.p
...
class ColumnwiseDropout(nn.Module):
    def __init__(self, p=0.5):
        super(ColumnwiseDropout, self).__init__()
        self.p = p

    def forward(self, x):
        if self.training:
            dropout_mask = torch.rand(1, x.size(1), 1, x.size(3), device=x.device) > self.p
            dropout_mask = dropout_mask / (1 - self.p)
            dropout_mask = dropout_mask.expand_as(x)
            return x * dropout_mask
        return x
...
def Linear(in_dim, out_dim, bias=True):
    """
    Creates a Linear layer with the bias initialized to zero if bias is True.

    Args:
        in_dim (int): The size of each input sample.
        out_dim (int): The size of each output sample.
        bias (bool): If set to `False`, the layer will not learn an additive bias.
                     Default: `True`

    Returns:
        An instance of nn.Linear with the bias initialized to zero.
    """
